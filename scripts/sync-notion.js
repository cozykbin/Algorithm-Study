...
async function addToNotion() {
  try {
    ...
    const author = process.env.PR_AUTHOR;   // GitHub 로그인 ID
    ...
    if (!match) {
      console.log('⚠️ PR 제목 형식이 맞지 않습니다.');
      console.log('형식: [아이디] O월 O일 문제풀이 (예: [cozykbin] 11월 18일 문제풀이)');
      return;
    }
    ...
    console.log(`📅 파싱된 정보: ${month}월 ${day}일, 멤버: ${memberId}, 날짜: ${dateStr}`);
    ...
    if (memberResponse.results.length > 0) {
      const memberPage = memberResponse.results[0];
      memberPageId = memberPage.id;
      memberName =
        memberPage.properties['이름'].title[0]?.plain_text || memberId;
      
      console.log(`✅ 멤버 찾음: ${memberName} (${author})`);
      
      // 멤버의 "이번 주 커밋" +1 & "오늘 풀이" 체크
      const currentCommits =
        memberPage.properties['이번 주 커밋']?.number || 0;
      ...
      console.log(
        `📊 ${memberName}의 이번 주 커밋: ${currentCommits} → ${
          currentCommits + 1
        }`
      );
    } else {
      console.log(
        `⚠️ Member DB에서 GitHub ID "${author}"를 찾을 수 없습니다. (속성명: GitHub ID)`
      );
    }
    
    // 2. Study Schedule DB에서 오늘 날짜의 문제 찾기
    ...
    console.log(
      `📚 오늘(${dateStr}) 문제: ${scheduleResponse.results.length}개 발견`
    );
    
    // 3. 오늘 문제에 멤버 추가 (릴레이션 컬럼: "해결한 멤버")
    for (const problem of scheduleResponse.results) {
      const problemTitle =
        problem.properties['문제'].title[0]?.plain_text || '제목 없음';
      
      if (!memberPageId) {
        console.log(
          `ℹ️ Member 페이지를 찾지 못해 "${problemTitle}"에 멤버를 추가할 수 없습니다.`
        );
        continue;
      }
      
      const existingMembers =
        problem.properties['해결한 멤버']?.relation || [];
      ...
      if (!memberIds.includes(memberPageId)) {
        ...
        console.log(
          `ℹ️ "${problemTitle}"에 ${memberName}이(가) 이미 등록되어 있습니다.`
        );
      }
    }
